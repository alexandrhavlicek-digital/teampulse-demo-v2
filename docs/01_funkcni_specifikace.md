...
- **Progress s auditní stopou (2026-07-30):** plnění cíle je sebe-report — nemění se volným posuvníkem, ale výhradně přes `GoalCheck.applyProgress` (js/goalcheck.js), která zapíše `goal.progressLog` {at, from, to, note, byId}. Vstupy: **kvartální check cílů** (todo na Přehledu, modal se všemi cíli, u každé změny povinný komentář „čím to dokládám"; záznam o provedení v kolekci `goalChecks`), pololetní check (komentář z formuláře), Copilot (komentář si vyžádá) a HR korekce v sekci Cíle (jediný ponechaný posuvník, v logu označená „Korekce HR"). Kadenci řídí HR v Pravidlech cyklu (`cycleConfig.goalCheck` = kvartálně / jen pololetí / vypnuto); běžící pololetní check ve stejném kvartálu todo nahrazuje. Historie plnění je u každého cíle (ikona hodin) a poslední změny s komentáři se ukazují manažerovi v Podkladech z období. KPI hodnoty (firemní/týmové) jsou oddělená veličina — spravuje je HR v KPI boardu (v produkci import z BI), zaměstnanec je nemění.
